...
// 1D DFT routines with OpenACC
void forward_dft_1d(Complex* psi_in, Complex* psi_out, int N) {
    #pragma acc parallel loop present(psi_in, psi_out)
    for (int k = 0; k < N; k++) {
        double sum_r = 0.0;
        double sum_i = 0.0;
        for (int t = 0; t < N; t++) {
...
void gpu_warmup() {
    ...
    std::vector<double>  warm_V(WARM_N, 1.0);
    std::vector<double>  warm_K(WARM_N, 1.0);
    ...
    double*  V     = warm_V.data();
    double*  K     = warm_K.data();

    #pragma acc data copy(psi[0:WARM_N]) create(psi_k[0:WARM_N]) copyin(V[0:WARM_N], K[0:WARM_N])
    {
        #pragma acc parallel loop
        for (int i = 0; i < WARM_N; i++) {
            double angle = -V[i] * 0.01;
        ...
        forward_dft_1d(psi, psi_k, WARM_N);

        #pragma acc parallel loop
        for (int i = 0; i < WARM_N; i++) {
            double angle = -K[i] * 0.01;
            double p_real = cos(angle);
            double p_imag = sin(angle);
        ...
        inverse_dft_1d(psi_k, psi, WARM_N);
    ...
    printf("GPU warm-up complete (ran 1 SSFM step on 1D grid %d)\n\n", WARM_N);
}
...
int main() {
    ...
    printf("  Split-Step Fourier Method — OpenACC Benchmark (1D NLSE)\n");
